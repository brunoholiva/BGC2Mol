...
def data_process(data_path):
    # Try to determine the separator by checking the file content
...
def generate_batch(data, batch_size, padding_token):
    ...
    for idx in range(0, len(data), batch_size):
        ...
        if idx + batch_size <= len(data):
            ...
            # Find max length for each sequence type (BGC features and SMILES)
            max_src_len = max(len(seq[0]) for seq in batch_data)
            max_tgt_len = max(len(seq[1]) for seq in batch_data)
            
            # Pad sequences to max length
            padded_batch = []
            for seq_pair in batch_data:
                src_seq, tgt_seq = seq_pair
                
                # Pad source sequence (BGC features)
                src_padded = np.concatenate([
                    SOS_token, 
                    src_seq, 
                    np.full(max_src_len - len(src_seq), padding_token, dtype=np.int64),
                    EOS_token
                ])
                
                # Pad target sequence (SMILES)
                tgt_padded = np.concatenate([
                    SOS_token,
                    tgt_seq,
                    np.full(max_tgt_len - len(tgt_seq), padding_token, dtype=np.int64),
                    EOS_token
                ])
                
                padded_batch.append([src_padded, tgt_padded])
            
            batches.append(np.array(padded_batch, dtype=object))
...
def fit(model, opt, loss_fn, train_dataloader, val_dataloader, epochs, patience, model_name):
    train_loss_list, validation_loss_list, train_acc_list, validation_acc_list = [], [], [], []
     ...
     # Try to determine the separator by checking the file content
    with open('./data/validation_data.csv', 'r') as f:
        first_line = f.readline()
        if '\t' in first_line:
            sep = '\t'
        else:
    ...
    df = pd.read_csv('./data/validation_data.csv', sep=sep)
    src_bgc_features, tgt_smiles = df['BGC_features'].values, df['SMILES'].values
